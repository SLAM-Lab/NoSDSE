...
static int task_counter[MAX_EDGE_NUM][MAX_EDGE_NUM];

//Used for simulation
double frame_start_time[MAX_EDGE_NUM][FRAME_NUM];
...
device_ctxt* deepthings_edge_init(uint32_t N, uint32_t M, uint32_t fused_layers, char* network, char* weights, uint32_t edge_id){
   ...
   int device_id = 0;
   int victim_id = 0;
   for(device_id = 0; device_id < MAX_EDGE_NUM; device_id++){
      for(victim_id = 0; victim_id < MAX_EDGE_NUM; victim_id++){
         task_counter[device_id][victim_id] = 0;
      }
   }
   return ctxt;
}
...
/*
   ...
   long long psec = os_model->profile->get_latency_in_psec("forward_partition", 
                      std::to_string(get_blob_frame_seq(temp)) + "_" + std::to_string(get_blob_task_id(temp)) + "_" + std::to_string((int)is_reuse));
   ...
   double sec = os_model->profile->get_latency_in_sec("forward_partition", 
                      std::to_string(get_blob_frame_seq(temp)) + "_" + std::to_string(get_blob_task_id(temp)) + "_" + std::to_string((int)is_reuse));
   std::cout << "In device " << os_model->node_id << ", frame:" << get_blob_frame_seq(temp) << ", task:" 
             << get_blob_task_id(temp) << ", reuse is:" << is_reuse << ", latency:" << sec << "s" << std::endl;
*/
   std::string input = std::to_string(get_blob_frame_seq(temp)) + "_" + std::to_string(get_blob_task_id(temp)) + "_" + std::to_string((int)is_reuse);
   record_static("forward_partition", (char*)input.c_str(), "application");
   sys_time_wait("forward_partition", (char*)input.c_str());
#if DATA_REUSE
   ...
   fclose(log);
#endif
   task_counter[ctxt->this_cli_id][get_blob_cli_id(temp)]++;
   (sim_ctxt.result)->set_edge_result(ctxt->this_cli_id, "edge_" + std::to_string(get_blob_cli_id(temp)) + "_task_number", 
                                      (double)task_counter[ctxt->this_cli_id][get_blob_cli_id(temp)]/(double)(get_blob_frame_seq(temp)+1));
}
...
void partition_frame_and_perform_inference_thread(void *arg){
...
#endif
   //printf("====================partition_frame_and_perform_inference_thread %f====================\n", sys_now_in_sec()-start_time);
   blob* temp;
   uint32_t frame_num;
   ...
   for(frame_num = 0; frame_num < FRAME_NUM; frame_num ++){
      ...
      /*recv_img()*/
      std::cout << "======================================= abs time is: " << sc_core::sc_time_stamp().to_seconds() << "======================================="<< std::endl;
      ...
      frame_start_time[ctxt->this_cli_id][frame_num] = sc_core::sc_time_stamp().to_seconds();
      while(1){
         temp = try_dequeue(ctxt->task_queue);
         if(temp == NULL) break;
         //printf("====================Processing task id is %d, data source is %d, frame_seq is %d at time %f====================\n", get_blob_task_id(temp), get_blob_cli_id(temp), get_blob_frame_seq(temp), sys_now_in_sec()-start_time);
...
void steal_partition_and_perform_inference_thread(void *arg){
   ...
   while(1){
      ...
      sprintf(logname, "edge_%d.log", ctxt->this_cli_id);
      FILE *log = fopen(logname, "a");
      if(os_model->node_id == 5)  fprintf(log, "steal_partition_and_perform_inference_thread : request gateway at time %f\n", sys_now_in_sec()-start_time);
      fclose(log);
#endif
      conn = connect_service(TCP, ctxt->gateway_local_addr, WORK_STEAL_PORT);
      ...
      sprintf(logname, "edge_%d.log", ctxt->this_cli_id);
      log = fopen(logname, "a");
      if(os_model->node_id == 5)  fprintf(log, "steal_partition_and_perform_inference_thread : request peer at time %f\n", sys_now_in_sec()-start_time);
      fclose(log);
#endif
      ...
      sprintf(logname, "edge_%d.log", ctxt->this_cli_id);
      log = fopen(logname, "a");
      if(os_model->node_id == 5)  fprintf(log, "steal_partition_and_perform_inference_thread : request reuse_data at time %f\n", sys_now_in_sec()-start_time);
      fclose(log);
#endif
      request_reuse_data(ctxt, temp, reuse_data_is_required);
      ...
      sprintf(logname, "edge_%d.log", ctxt->this_cli_id);
      log = fopen(logname, "a");
      if(os_model->node_id == 5)  fprintf(log, "steal_partition_and_perform_inference_thread : process at time %f\n", sys_now_in_sec()-start_time);
      fclose(log);
#endif
      ...
      sprintf(logname, "edge_%d.log", ctxt->this_cli_id);
      log = fopen(logname, "a");
      if(os_model->node_id == 5)  fprintf(log, "steal_partition_and_perform_inference_thread : finish processing at time %f\n", sys_now_in_sec()-start_time);
      fclose(log);
#endif
      free_blob(temp);
   }
...
void* steal_client_reuse_aware(void* srv_conn, void* arg){
   ...
   sprintf(logname, "edge_%d.log", ctxt->this_cli_id);
   FILE *log = fopen(logname, "a");
   fprintf(log, "Hand out local task %d:%d, size is %d at time %f\n", get_blob_cli_id(temp), get_blob_task_id(temp), temp->size, sys_now_in_sec()-start_time); 
   fclose(log);
#endif
...
#if DEBUG_LOG
   double t1 = sys_now_in_sec();
#endif
   send_data(temp, conn);
#if DEBUG_LOG
   sprintf(logname, "edge_%d.log", ctxt->this_cli_id);
   log = fopen(logname, "a");
   fprintf(log, "Transmitting duration is %f\n", sys_now_in_sec()-t1); 
   fclose(log);
#endif
#if DEBUG_COMMU_SIZE
